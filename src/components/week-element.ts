 ...
 */
@customElement('week-element')
export class WeekElement extends LitElement {
    ...
    render() {
        return this._planTask.render({
            ...
            complete: (overallResponse) => {
                ...
                // const pageValue = overallResponse.find((value: any) => {
                //     return isThisWeek(value.startDate);
                // });
                // for the example just always show the same week
                const pageValue = overallResponse[0];

                if (pageValue !== undefined) {
                    return html`
                        <h1>${this.titleValue}</h1>
                        <div class="week">
                            <h3>${pageValue.week} of 16</h3>
                            <div class="schedule">
                                ${pageValue.schedule.map(
                                    (day: any) => html`
                                        <div
                                            class="day"
                                            ${animate({
